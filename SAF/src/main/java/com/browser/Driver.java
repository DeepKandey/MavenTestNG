...
/**
 * 
 * @author asakthiv Driver class is used to start browsers based on the property
 *         file input. User gets the option to work on firefox and chrome
 *         browser. Private constructor to avoid external initialisation -
 *         SingletonPattern is achieved
 */
	...
	private void startBrowserForLocal() {
		...
		try {
			if (browser.equalsIgnoreCase("chrome")) {
				// WebDriverManager.chromedriver().setup(); //WebDriverManager some time wont
				// work because of proxy issues
				System.setProperty("webdriver.chrome.driver", Constants.CHROMEDRIVERPATH);
				driver = new ChromeDriver();
			} else if (browser.equalsIgnoreCase("firefox")) {
				// WebDriverManager.firefoxdriver().setup(); //WebDriverManager some time wont
				// work because of proxy issues
